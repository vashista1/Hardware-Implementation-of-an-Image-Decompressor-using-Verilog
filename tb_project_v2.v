...
project uut (
		...
		.UART_RX_I(1'b1), //this should be a 1 because 
		.UART_TX_O()
...
initial begin
	...
	wait (uut.M1_start != 0);	//this assumes S_IDLE is the first in the list where the states are enumerated
	$write("Starting Decoder at %t\n\n", $realtime);
	wait (uut.M1_end != 0);	//this assumes we go back to S_IDLE when we are done
